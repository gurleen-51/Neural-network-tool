...
def landing_page():
    ...
    with col1:
        if st.button("📐", key="landing_math"):
            st.session_state.page = "math"
        st.markdown('''
            <div class="card">
                <div class="card-title">📐</div>
                <div class="card-desc">Step-by-step math for Perceptron & MLP</div>
            </div>''', unsafe_allow_html=True)

    with col2:
        if st.button("🧠", key="landing_playground"):
            st.session_state.page = "playground"
        st.markdown('''
            <div class="card">
                <div class="card-title">🧠</div>
                <div class="card-desc">Upload dataset, train & experiment</div>
            </div>''', unsafe_allow_html=True)

    with col3:
        if st.button("📊", key="landing_visual"):
            st.session_state.page = "visual"
        st.markdown('''
            <div class="card">
                <div class="card-title">📊</div>
                <div class="card-desc">View network, loss & predictions</div>
            </div>''', unsafe_allow_html=True)
